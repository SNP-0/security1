...
public class PrincipalOauth2UserService extends DefaultOAuth2UserService {
  ...
  public PrincipalOauth2UserService(UserMapper usermapper, PasswordEncoder passwordEncoder) {
  ...
  public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
    ...
    if (userEntity == null) {
      ...
      userEntity = UserDto.builder().username(username).password(password).email(email).role(role)
          .provider(provider).providerId(providerId).build();

      usermapper.insertMember(userEntity);
    } else {
      System.out.println("로그인을 이미 한 적이있습니다. 당신은 자동 회원가입이 되어있습니다");
    }

    return new PrincipalDetails(userEntity, oauth2User.getAttributes());// OAuth2User
  }
}
